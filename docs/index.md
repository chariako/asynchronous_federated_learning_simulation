...
=== "Core FL Capabilities"
    ...
    - **Standard Benchmark Implementation**: Equivalent versions under AFL-Sim's architecture can be recovered for several standard FL and distributed SGD algorithms by appropriately choosing the configuration parameters. For a full list of supported algorithms, please refer to the list of [Implemented Algorithms](user_guide/configuration.md#implemented-algorithms) in [Setting Up a YAML Configuration](user_guide/configuration.md).

=== "Datasets & Models"

    - **Benchmarking Tasks**: Composed of select TorchVision datasets and curated vision models. For a full list of available options, refer to [Setting Up a YAML Configuration](user_guide/configuration.md).
...
=== "`pip`"
    !!! tip
        Create and activate a virtual environment before installing, e.g., `python -m venv .venv && source .venv/bin/activate`.
...
!!! tip
    If you installed the package using uv, prepend `uv run` to all `afl-sim` CLI commands below to perform the execution in the isolated environment.
...
=== "Python"
    ...
    ```python title="launch_simulation.py" linenums="1"
    from afl_sim import run_simulation

    run_simulation(config="path/to/config.yaml", output_dir="path/to/output_dir")
    ```

For an annotated YAML configuration template and a comprehensive guide on configuring your simulation parameters, check [Setting Up a YAML Configuration](user_guide/configuration.md).
...
=== "Python"
    ...
    ```python title="resume_simulation.py" linenums="1"
...
To learn more about using and extending AFL-Sim, explore the following resources:

* **[Setting Up a YAML Configuration](user_guide/configuration.md)**: Read up on creating YAML configuration files and tuning their parameters, and review the supported options for hardware acceleration, models, datasets and implemented algorithms.
* **[Using the Python API](user_guide/python_api.md)**: Learn how to construct custom configuration objects in lieu of YAML files, and use them to launch new simulations inside a Python script.
* **[Execution Guide](user_guide/execution.md)**: Dive deeper into the run and resume functionalities of AFL-Sim, the types of artifacts AFL-Sim produces, and how it manages directories and storage.
* **[Implementation Notes](implementation/index.md)**: Learn more about how AFL-Sim works under the hood.
* **[CLI Reference](reference/cli.md)**: Read through the documentation for AFL-Sim's CLI commands, including input arguments, options and defaults.
* **[API Reference](reference/api.md)**: Review the documentation for AFL-Sim's user API, including all importable modules and their arguments.
